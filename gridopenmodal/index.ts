...
export class gridopenmodal implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    onNavigate = (item?: ComponentFramework.PropertyHelper.DataSetApi.EntityRecord): void => {
        if (item) {
           //@ts-ignore
           this.context.navigation.navigateTo({pageType:"entityrecord", entityName: item.getNamedReference()._etn,entityId:item.getNamedReference()._id}, {
                target: 2,  
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): void {
        ...
        if(dataset.records && !this.records){
